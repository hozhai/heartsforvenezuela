<script lang="ts">
	...
	let {
		text = 'Hello World',
		colors = { first: '#FFCC00', second: '#00247D', third: '#CF142B' },
		sparklesCount = 10,
		className = '',
		sparkles = []
	} = $props();
	...
	let generateStar = () => {
		...
		if (colorRandom >= 0.666) {
			color = colors.first;
		} else if (colorRandom < 0.666 && colorRandom >= 0.333) {
			color = colors.second;
		} else {
			color = colors.third;
		}
	...
	onMount(async () => {
		initializeStars();
		interval = setInterval(updateStars, 400);
	});
...
<div
	...
	style:--sparkles-third-color="{colors.third};"
